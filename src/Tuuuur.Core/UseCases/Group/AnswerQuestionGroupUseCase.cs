    ...
    : ADbUseCase<AnswerQuestionGroupPartyRequest, EmptyResponse>(p_Logger, p_UnitOfWork)
{
    ...
    {
        string v_PartieCode = await p_CacheService.GetAsync<string>(RedisKeys.User.UserGroup(p_Request.UserId), p_CancellationToken);
        ...
        GroupParty v_Party = await p_CacheService.GetAsync<GroupParty>(RedisKeys.Group.ByCode(v_PartieCode), p_CancellationToken);
        ...
        UserPartyQuestion v_UserPartyQuestion = await p_CacheService.GetAsync<UserPartyQuestion>(RedisKeys.Group.QuestionUserAnswer(v_Party.Code, v_Question.Id, p_Request.UserId), p_CancellationToken: p_CancellationToken);
        v_UserPartyQuestion.IdAnswer = p_Request.AnswerId;
        v_UserPartyQuestion.DtAnsweredAt = DateTime.Now;

        await p_CacheService.SetAsync(RedisKeys.Group.QuestionUserAnswer(v_Party.Code, v_Question.Id, p_Request.UserId), v_UserPartyQuestion, p_CancellationToken: p_CancellationToken);

        // Add user to the answered set for early round termination
        await p_CacheService.SetAddAsync(RedisKeys.Group.QuestionAnswered(v_Party.Code, v_Question.Id), p_Request.UserId, p_CancellationToken: p_CancellationToken);

        // Check if all players have answered
        long v_AnsweredCount = await p_CacheService.SetLengthAsync(RedisKeys.Group.QuestionAnswered(v_Party.Code, v_Question.Id), p_CancellationToken);
        long v_TotalPlayers = await p_CacheService.SetLengthAsync(RedisKeys.Group.Users(v_Party.Code), p_CancellationToken);
        ...
        if (v_AnsweredCount >= v_TotalPlayers)
        {
            ...
            await p_CacheService.PublishAsync(
                RedisKeys.Group.QuestionAllAnsweredChannel(v_Party.Code, v_Question.Id),
                true,
                p_CancellationToken
            );
        }
        
        List<User> v_UserInParty = await p_CacheService.SetMembersAsync<User>(RedisKeys.Group.Users(v_PartieCode), p_CancellationToken: p_CancellationToken);
