    ...
    : ADbUseCase<RankedLogicRequest, EmptyResponse>(p_Logger, p_UnitOfWork)
{
        ...
        CancellationToken p_CancellationToken)
    {
        ...
        bool v_IsP1Connected = await p_CacheService.GetAsync<bool>(RedisKeys.User.UserConnected(v_Player1.Id), p_CancellationToken);
        bool v_IsP2Connected = await p_CacheService.GetAsync<bool>(RedisKeys.User.UserConnected(v_Player2.Id), p_CancellationToken);
        if (!v_IsP1Connected && !v_IsP2Connected)
        {
            return await CancelPartyLogicAsync(v_Party, v_Player1, v_Player2, p_CancellationToken);
        }        
        if (!v_IsP1Connected)
        {
    ...
    /// <summary>
    /// Cancels the party without Elo changes when both players disconnect.
    /// </summary>
    private async Task<EmptyResponse> CancelPartyLogicAsync(Party p_Party, User p_Player1, User p_Player2, CancellationToken p_CancellationToken)
    {
        ...
        foreach (PartyUser v_PartyUser in p_Party.PartyUsers)
        {
            // No Elo changes, no winner
            v_PartyUser.Elo = 0;
            v_PartyUser.Winner = false;
            v_PartyUser.FinalScore = 0;
        }
        IMappingAddEntity<PartyBase, IEntity> v_MappingAddEntity = await m_UnitOfWork.PartyRepository.CreatePartyAsync(p_Party, p_CancellationToken);
        m_UnitOfWork.Save();
        try
        {
            // Save user party questions
            if (v_MappingAddEntity.MapBoEntity.PartyQuestions != null)
            {
                foreach (PartyQuestion v_PartyQuestion in v_MappingAddEntity.MapBoEntity.PartyQuestions)
                {
                    foreach (User v_User in new[] { p_Player1, p_Player2 })
                    {
                        UserPartyQuestion v_UserPartyQuestion = await p_CacheService.GetAsync<UserPartyQuestion>(
                            RedisKeys.Ranked.QuestionUserAnswer(v_OriginalPartyId, v_PartyQuestion.IdQuestion, v_User.Id),
                            p_CancellationToken);
        ...
        finally
        {
            ...
            await p_CacheService.RemoveByPatternAsync(RedisKeys.Ranked.ById(v_OriginalPartyId) + "*", p_CancellationToken: p_CancellationToken);
            if (p_Player1 != null)
                await p_CacheService.RemoveByPatternAsync(RedisKeys.User.UserRanked(p_Player1.Id), p_CancellationToken: p_CancellationToken);
            if (p_Player2 != null)
                await p_CacheService.RemoveByPatternAsync(RedisKeys.User.UserRanked(p_Player2.Id), p_CancellationToken: p_CancellationToken);
        }
        return new EmptyResponse();
    }
    /// <summary>
