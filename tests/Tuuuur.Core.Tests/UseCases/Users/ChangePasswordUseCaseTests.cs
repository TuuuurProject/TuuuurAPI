...
public class ChangePasswordUseCaseTests
{
    ...
    public ChangePasswordUseCaseTests()
    {
        ...
        m_MockLogger = m_MockRepository.Create<ILogger<ChangePasswordUseCase>>();
        m_MockUserRoleServiceMock = m_MockRepository.Create<IUserRoleService>();
        m_MockMediator = m_MockRepository.Create<IMediator>();
    }

    private ChangePasswordUseCase CreateUseCase()
    {
        return new ChangePasswordUseCase(
            m_MockUnitOfWork.Object,
            m_MockLogger.Object,
            m_MockMediator.Object,
    ...
    public async Task Handle_ExpectedBehaviorAsync()
    {
        ...
        ChangePasswordUseCase v_UseCase = CreateUseCase();

        User v_User = BoFactory.CreateUser();
        
        ChangePasswordRequest v_Request = new("Mot2Passe","SuperPassword");
        ...
        m_MockMediator.Setup(p_Uow => p_Uow.Send(new HashRequest("Mot2Passe"), It.IsAny<CancellationToken>())).ReturnsAsync(new StringResponse(v_User.Password));
        m_MockMediator.Setup(p_Uow => p_Uow.Send(new HashRequest("SuperPassword"), It.IsAny<CancellationToken>())).ReturnsAsync(new StringResponse("MonSuperStringHash"));
        ...
        v_User.Password = "MonSuperStringHash";
