...
public class HistoryControllerTests
{
    ...
    public async Task GetHistoryAsync_ReturnsOkObjectResult()
    {
        ...
        m_MediatorMock.Setup(p_P => p_P.Send(It.IsAny<GetAllHistoryRequest>(), It.IsAny<CancellationToken>())).ReturnsAsync(new GenericEntityResponse<History>(new History()));
            
        // Act
        IActionResult v_Result = await m_Controller.GetHistoryListAsync(v_Request, new PaginationRequestValidator(), new GenericEntityPresenter<History>(), CancellationToken.None);
